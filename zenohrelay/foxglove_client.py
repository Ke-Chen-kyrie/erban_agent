...
FOXGLOVE_BRIDGE_URL = env("FOXGLOVE_URL", "ws://localhost:8765")
ROSBRIDGE_URL = env("ROSBRIDGE_URL", "ws://192.168.217.100:9090")
...
class FoxgloveClient:
    ...
    async def connect(self, discovery_timeout: float = 3.0):
        ...
        # 使用 legacy client: 不读取环境代理直接连 bridge，且请求头更朴素，
        # 与 foxglove-sdk-cpp 握手兼容（见 debug-tool）。compression=None 禁用
        # 压缩扩展，避免部分 bridge 版本协商失败。
...
class FoxgloveImageCapture:
    ...
    def read_next(
        ...
        """等待比 ``after_sequence`` 新的帧。

        返回 ``(success, RGB image, sequence, ROS timestamp_ns)``。普通调用方继续
        使用 :meth:`read`；中继服务使用本方法避免重复识别同一缓存帧。
        """
        if not self._running:
            self._connect_sync()
        deadline = time.monotonic() + max(0.0, timeout)
...
def decode_rosbridge_image(msg: dict) -> Optional[np.ndarray]:
    """解码 rosbridge 的 sensor_msgs/CompressedImage 消息为 RGB 数组."""
    import base64
    import cv2

    try:
        data = msg.get("data") if msg else None
        if not data:
            return None
        jpeg = np.frombuffer(base64.b64decode(data), np.uint8)
        img = cv2.imdecode(jpeg, cv2.IMREAD_COLOR)
        return None if img is None else cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
    except Exception as e:
        logger.error(f"[rosbridge] 图像解码失败: {e}")
        return None


class RosbridgeImageCapture:
    """从 ROS1 rosbridge_server（WebSocket，默认端口 9090）读取最新一帧压缩图像.

    接口与 FoxgloveImageCapture 一致（含 read_next），只支持
    ``sensor_msgs/CompressedImage``（base64 JPEG）。
    """

    SUBSCRIBE_TYPE = "sensor_msgs/CompressedImage"

    def __init__(self, topic: str, url: str = None):
        self._topic = topic
        self._url = url or ROSBRIDGE_URL
        self._ws = None
        self._latest_frame: Optional[np.ndarray] = None
    ...
    def read(self, timeout: float = 1.0) -> Tuple[bool, Optional[np.ndarray]]:
        """读取最新一帧图像，返回 (success, RGB ndarray)."""
        if not self._running:
            self._connect_sync()
        deadline = time.monotonic() + max(0.0, timeout)
        with self._condition:
    ...
    def read_next(
        ...
        返回 ``(success, RGB image, sequence, timestamp_ns)``。中继服务使用
        本方法避免重复处理同一缓存帧。
        """
        if not self._running:
            self._connect_sync()
        ...
        for _ in range(50):
            with self._condition:
                if self._latest_frame is not None:
                    return
            time.sleep(0.1)
        logger.warning(f"[rosbridge] 5 秒内未收到话题 '{self._topic}' 的帧")
    ...
    async def _async_main(self):
        try:
            ...
            # 注意: 不要带 fragment_size=0 —— 实测会让这台 rosbridge_server 直接不投递任何消息
            #（带 fragment_size=0 订阅 /rosout 一条都收不到，去掉后立即可收）。
            # 不指定 fragment_size 时服务端按默认行为发送，大消息走下方 fragment 重组兜底。
            await self._ws.send(json.dumps({
                "op": "subscribe",
                "topic": self._topic,
            ...
            while self._running:
                ...
                if msg.get("op") == "fragment":
                    fid = msg.get("id")
                    self._fragments[fid] = self._fragments.get(fid, "") + msg.get("data", "")
                    try:
                        msg = json.loads(self._fragments.pop(fid))
                    except json.JSONDecodeError:
                        continue

                if msg.get("op") != "publish" or msg.get("topic") != self._topic:
                    continue

                img = decode_rosbridge_image(msg.get("msg"))
                if img is not None:
                    with self._condition:
                        if not self._running:
                            return
            ...
            logger.error(f"[rosbridge] 错误: {e}")
        finally:
            if self._ws is not None:
                try:
                    await self._ws.send(json.dumps({"op": "unsubscribe", "topic": self._topic}))
                except Exception:
                    pass
                try:
                    await self._ws.close()
