...
from webinfer.memory_summarizer import SummarizerModel
from prompt import GESTURE_DETECT_PROMPT, EVENT_LABELS, build_detect_prompt, CONFIRMABLE_EVENTS, EVENT_CONFIRM_PROMPTS, confirm_seconds
...
def _print_vlm_messages(messages: list[dict[str, Any]], model_name: str, generation_kwargs: dict[str, Any]) -> None:
...
def _get_i18n(language: str = "en") -> dict[str, str]:
    ...
    return {
        "user_query_header": USER_QUERY_HEADER_ZH,
...
DEFAULT_SYSTEM_PROMPT_EN = GESTURE_DETECT_PROMPT
...
def _iso_time(epoch_ts: Optional[float]) -> str:
    """Epoch 秒 → 带时区的 ISO8601 毫秒字符串；空值返回空串。"""
    if not epoch_ts:
        return ""
    return datetime.fromtimestamp(epoch_ts).astimezone().isoformat(timespec="milliseconds")
...
def _get_response_frame_indices(messages: list[dict[str, Any]]) -> list[int]:
    ...
    for message in messages:
        if message.get("role") == "user":
            ...
            if frame_idx >= 0 and extract_event(str(message.get("content", ""))):
                indices.append(frame_idx)
    return indices


_EVENT_LABEL_SET = frozenset(EVENT_LABELS)

# 事件标签 → 推送优先级：默认 alert（报警），普通手势用 info（普通提醒）
_EVENT_LEVELS = {
    "ok": "info",
    "thumbs_up": "info",
}

# 普通手势 → agent 动作管道（event_type="action"）的动作名映射。
# 命中的标签推 {"event_type":"action","actions":[...]} 走 agent 的无声手势管道；
# 未命中的标签（身体不适/环境/跌倒等）推通用系统事件，event_type 用英文标签。
ACTION_EVENT_NAMES = {
    "wave": "waving",
    "ok": "ok_sign",
    "thumbs_up": "thumbs_up",
...
# 需要二次确认的事件集合（与 prompt.CONFIRMABLE_EVENTS 对应）
CONFIRMABLE_EVENT_SET = frozenset(CONFIRMABLE_EVENTS)

# 匹配确认 JSON 中的 "confirm": true/false（可能有 thinking 前缀，取最后一次出现）
_CONFIRM_BOOL_RE = re.compile(r'"confirm"\s*:\s*(true|false)')


def _extract_confirm_bool(text: str) -> Optional[bool]:
    """从模型输出中解析最终的确认结果；有 thinking 时取最后一次出现的值。"""
    matches = list(_CONFIRM_BOOL_RE.finditer(text or ""))
    if not matches:
        return None
    return matches[-1].group(1).lower() == "true"


def _system_event_label_allowed(label: str, configured_labels: str) -> bool:
    """Return whether a detected label may be pushed to SYSTEM_EVENT_URL.

    An empty configuration preserves the original behavior (all whitelisted
    events). A comma-separated value such as ``ok,fall`` acts as an allowlist.
    """
    allowed = {
        item.strip()
        for item in str(configured_labels or "").split(",")
        if item.strip()
    }
    return not allowed or label in allowed

_EVENT_LINE_RE = re.compile(r"^\s*[\[(]?\s*([a-z][a-z_]*)\s*[\])]?\s*(?:[:：]|\s+)\s*(.+)$")


def _extract_label_from_line(line: str) -> tuple[str, str]:
    """从一行里抓白名单英文标签，兼容 '捂胸 (chest_pain) 描述' / 'fall 描述' 等格式。

    返回 (label, desc)；找不到则 ('', '')。desc 会去掉标签本身及多余标点/空白。
    """
    for m in re.finditer(r"[a-z][a-z_]*", line):
        label = m.group(0)
        if label not in _EVENT_LABEL_SET:
            continue
...
def _parse_event_lines(text: str) -> list[tuple[str, str]]:
    """Parse '标签：描述' lines (accepts '：' or ':', optional brackets).

    Mirrors reference project's parse_actions: one event per line, label first,
    only whitelisted labels survive. Off-label / prose lines are dropped.
    """
    events: list[tuple[str, str]] = []
    body = str(text or "")
    m = re.search(r"<[/]?\s*response\s*>", body)
    if m:
        body = body[m.end():]
    for line in body.splitlines():
        ...
        if match:
            label, desc = match.group(1).strip(), match.group(2).strip()
        else:
            label, desc = _extract_label_from_line(line)
        if label in _EVENT_LABEL_SET and desc:
            events.append((label, desc))
    return events


def _find_json_object(text: Any) -> Optional[dict]:
    """提取输入中第一个完整的 {...} JSON 对象；失败返回 None。"""
    s = str(text or "")
    i = s.find("{")
    while i != -1:
        depth = 0
...
def extract_event(text: Any) -> Optional[tuple[str, str, str]]:
    """从模型输出解析 (label, desc, name)。优先 JSON，失败回落宽容标签解析。"""
    obj = _find_json_object(text)
    if obj is not None:
        ev = obj.get("event", obj.get("label"))
        if ev is None:
            return None
        ev_s = str(ev).strip()
        if not ev_s or ev_s.lower() in ("null", "none"):
            return None
        desc = obj.get("desc", obj.get("description", ""))
        name = obj.get("name", obj.get("person", obj.get("user", "")))
        return ev_s, str(desc or "").strip(), str(name or "").strip()
    events = _parse_event_lines(text)
    if events:
        label, desc = events[0]
        return label, desc, ""
    return None
...
def is_silence_output(text: str) -> bool:
    """判断是否为“无事件”输出（JSON null 或标签式 </silence>）。"""
    obj = _find_json_object(text)
    if obj is not None:
        ev = obj.get("event", obj.get("label"))
        return ev is None
    s = (text or "").strip()
    return s.startswith("</silence>") or s.startswith("<silence>")


def normalize_model_output(text: str) -> str:
    """把模型输出规范化为单行 JSON：有事件 {"event":...}，无事件 {"event": null}。"""
    raw = (text or "").replace("\r\n", "\n").replace("\r", "\n").strip()
    if not raw:
        return '{"event": null}'
    ev = extract_event(raw)
    if ev and ev[0] in _EVENT_LABEL_SET:
        payload: dict[str, Any] = {"event": ev[0], "desc": ev[1]}
        if ev[2]:
            payload["name"] = ev[2]
...
def extract_response_payload(text: str) -> Optional[str]:
    """返回白名单事件的标准 JSON 字符串；无/非白名单事件返回 None。"""
    ev = extract_event(text)
    if not ev or ev[0] not in _EVENT_LABEL_SET:
        return None
    payload: dict[str, Any] = {"event": ev[0], "desc": ev[1]}
    if ev[2]:
        payload["name"] = ev[2]
...
class AdapterConfig:
    ...
    # 主 VLM 图片分辨率上限（像素数）。0 = 不缩放，发送原始分辨率。
    ...
    main_disable_thinking: bool = False
    summary_disable_thinking: bool = True
    ...
    debug_print_vlm_messages: bool = False
    frame_save_dir: str = "/tmp/streaming_adapter_frames"
    language: str = "en"
    system_prompt: str = DEFAULT_SYSTEM_PROMPT_EN
    system_event_url: str = os.environ.get("SYSTEM_EVENT_URL", "http://127.0.0.1:8770/system_event")
    agent_webhook_url: str = os.environ.get("AGENT_WEBHOOK_URL", "http://192.168.217.100:8769/event")
    system_event_labels: str = os.environ.get("SYSTEM_EVENT_LABELS", "")
    event_dedup_seconds: float = 5.0
    time_api_url: str = ""
    night_time_start: int = 22
    night_time_end: int = 6
    event_confirm_enabled: bool = True
    event_confirm_seconds: dict[str, float] = field(default_factory=dict)
    langfuse_chunk_record_enabled: bool = True
...
class SessionState:
    ...
    long_term_compression_task: Optional[asyncio.Task] = field(default=None, repr=False)
    ...
    _last_event_push: dict[str, float] = field(default_factory=dict)
    _confirm_pending: set[str] = field(default_factory=set, repr=False)
    latest_image: Optional[str] = field(default=None, repr=False)
    langfuse_pushed_events: dict[int, list[dict[str, Any]]] = field(default_factory=dict, repr=False)


class StreamingInferAdapter:
    def __init__(self, config: AdapterConfig):
        self.config = config
        self._summary_log_lock = threading.Lock()
        ...
        if config.enable_summarizer:
            self.summarizer = SummarizerModel(
                ...
                disable_thinking=config.summary_disable_thinking,
        ...
        if config.summary_log_path:
            Path(config.summary_log_path).expanduser().parent.mkdir(parents=True, exist_ok=True)
        self._remote_time_offset: float = 0.0
        if config.time_api_url:
            asyncio.ensure_future(self._sync_time())

    async def _sync_time(self) -> None:
        """Sync time with remote API to correct for inaccurate robot clock."""
        try:
            import aiohttp
            async with aiohttp.ClientSession() as session:
                async with session.get(
                    self.config.time_api_url.strip(),
                    ...
                    if resp.status == 200:
                        ...
                        remote_str = (
                            data.get("datetime")
                            or data.get("utc_datetime")
                            or data.get("unixtime")
                        )
                        if remote_str:
                            if isinstance(remote_str, (int, float)):
                                remote_ts = float(remote_str)
                            else:
                                remote_dt = datetime.fromisoformat(
                                    str(remote_str).replace("Z", "+00:00")
                                )
                                remote_ts = remote_dt.timestamp()
                            local_ts = time.time()
                            self._remote_time_offset = remote_ts - local_ts
                            LOGGER.info(
                                "time synced: remote_offset=%.1fs local=%s remote=%s",
                                self._remote_time_offset,
                                datetime.fromtimestamp(local_ts).isoformat(),
                                datetime.fromtimestamp(remote_ts).isoformat(),
                            )
        except Exception:
            LOGGER.warning("time sync failed, using local time")

    def _get_time_period(self) -> str:
        """Return current time period label (夜间/上午/午间/下午/晚间)."""
        now_ts = time.time() + self._remote_time_offset
        hour = datetime.fromtimestamp(now_ts).hour
        night_start = self.config.night_time_start
        night_end = self.config.night_time_end
        ...
        for sid in expired:
            ...
            if state is not None:
                for job in state.async_pending_summary_jobs:
                    job["task"].cancel()
                if state.long_term_compression_task is not None:
                    state.long_term_compression_task.cancel()
    ...
    async def handle_reset(self, request: web.Request) -> web.Response:
        ...
        if removed_state is not None:
            for job in removed_state.async_pending_summary_jobs:
                job["task"].cancel()
            if removed_state.long_term_compression_task is not None:
                removed_state.long_term_compression_task.cancel()
    ...
    async def _handle_chat_payload(
        ...
        *,
        ...
        captured_at_values = _extract_frame_captured_at_values(request, payload, len(image_refs))

        image_paths = [self._resolve_frame_ref(ref, state) for ref in image_refs]
        if image_paths:
            state.latest_image = str(image_paths[-1])
        LOGGER.info(
        ...
        if (
            ...
            await self._record_flushed_chunk_if_event(state)
            if (
                self._async_summary_enabled()
        ...
        for tr, ip, captured_at in zip(time_ranges, image_paths, captured_at_values):
            state.frame_count += 1
            state.current_chunk["image_paths"].append(str(ip))
            state.current_chunk["frame_time_ranges"].append(tr)
            state.current_chunk["frame_captured_at"].append(captured_at)
            state.current_chunk["summarizer_frame_cache"].append(
                {"path": str(ip), "captured_at": captured_at}
            )
        ...
        turn_input_record = {
            ...
            "frame_captured_at": list(state.current_chunk["frame_captured_at"]),
        }
        is_forced_silence = (
        ...
        if is_forced_silence:
            ...
            # Log when model output is suppressed (non-whitelisted event dropped)
            if extract_event(raw_text or "") and not extract_event(generated_text):
                suppressed = (raw_text or "").strip()[:120]
                print(f"\033[90m[suppressed] {suppressed}\033[0m", flush=True)

        self._execute_pending_qa_archive(state)

        event = extract_event(generated_text)
        if event and state.current_query_text:
            response_record: dict[str, Any] = {"event": event[0], "desc": event[1]}
            if event[2]:
                response_record["name"] = event[2]
            state.current_chunk["response_records"].append(
                (time_range, json.dumps(response_record, ensure_ascii=False))
            )
        if (
            event
            and event[0] in _EVENT_LABEL_SET
            and (self.config.system_event_url or self.config.agent_webhook_url)
            and _system_event_label_allowed(event[0], self.config.system_event_labels)
        ):
            label, desc, person_name = event[0], event[1], event[2]
            now_ts = time.time()
            last_ts = state._last_event_push.get(label, 0)
            if now_ts - last_ts < self.config.event_dedup_seconds:
                LOGGER.debug("suppressed duplicate event [%s] (last pushed %.1fs ago)", label, now_ts - last_ts)
            elif label in CONFIRMABLE_EVENT_SET and self.config.event_confirm_enabled:
                self._maybe_start_confirm(state, label, desc, person_name, state.chunk_index)
            else:
                state._last_event_push[label] = now_ts
                event_images = [str(ip) for ip in image_paths]
                if self.config.system_event_url:
                    asyncio.ensure_future(self._push_system_event(label, desc, person_name, event_images))
                if self.config.agent_webhook_url:
                    asyncio.ensure_future(self._push_event_to_agent(label, desc, person_name, event_images))
                print(f"\033[91m[SYSTEM_EVENT] [{label}] {desc}\033[0m", flush=True)
                self._mark_langfuse_event_pushed(
                    state, state.chunk_index, label, desc, person_name
                )
    ...
    def _build_internal_user_message(
        ...
        *,
        ...
        if batch_time_marker:
            ...
            content.append({"type": "text", "text": f"<{batch_time_marker} [{period}]>"})
    ...
    @staticmethod
    def _truncate_prefix(prefix: str, max_chars: int) -> str:
        if max_chars <= 0 or len(prefix) <= max_chars:
            return prefix
        return "[...truncated...]\n" + prefix[-(max_chars - len("[...truncated...]\n")):]
    ...
    def _build_main_internal_messages(
        ...
        dynamic_content = build_dynamic_system_content(
            ...
            language=self.config.language,
            max_qa_entries=self.config.max_qa_entries,
        )
        prefix_content = "\n\n".join(
            part for part in (static_content, dynamic_content) if part
        )
        prefix_content = self._truncate_prefix(prefix_content, self.config.max_prefix_chars)
    ...
    def _resolve_system_prompt(self, payload: Optional[dict[str, Any]] = None) -> str:
        ...
        if not prompt or prompt == DEFAULT_SYSTEM_PROMPT_EN:
            prompt = build_detect_prompt(
                self.config.frame_seconds,
                self.config.night_time_start,
                self.config.night_time_end,
                self.config.chunk,
            )
        return prompt

    def _confirm_delay(self, label: str) -> float:
        """返回某事件二次确认的延迟秒数（配置可覆盖 prompt 默认）。"""
        return float(self.config.event_confirm_seconds.get(
            label, confirm_seconds(label)
        ))

    def _maybe_start_confirm(
        self,
        state: SessionState,
        label: str,
        desc: str,
        ...
        """Start a one-shot confirmation (delay + single-frame recheck).

        The first detection of a label starts the confirmation task; any further
        detections of the same label while a confirmation is pending (or within
        the existing event dedup window) are swallowed and must NOT start a new
        confirmation.
        """
        if label in state._confirm_pending:
            LOGGER.debug("[%s] confirm already pending; swallow duplicate", label)
            return
        if (
            time.time() - state._last_event_push.get(label, 0)
            < self.config.event_dedup_seconds
        ):
            LOGGER.debug("[%s] confirmed+pushed recently; swallow", label)
            return
        state._confirm_pending.add(label)
        delay = self._confirm_delay(label)
        print(
            f"\033[91m[{label.upper()}_DETECTED] {desc} -> confirm in {delay:.1f}s\033[0m",
            flush=True,
        )
        asyncio.ensure_future(self._run_confirm(state, label, desc, person_name, chunk_index))
    ...
    async def _run_confirm(
        ...
        """Wait `_confirm_delay` seconds, recheck the freshest frames, then push or drop."""
        try:
            await asyncio.sleep(self._confirm_delay(label))
            image_refs = self._confirm_frame_refs(state)
            confirmed = await self._confirm_event(label, image_refs)
            if not confirmed:
                LOGGER.info("[%s] confirm REJECTED, event dropped: %s", label, desc)
                print(f"\033[93m[{label.upper()}_CONFIRM] rejected, drop: {desc}\033[0m", flush=True)
                return
            state._last_event_push[label] = time.time()
            event_images = image_refs
            if self.config.system_event_url:
                await self._push_system_event(label, desc, person_name, event_images)
            if self.config.agent_webhook_url:
                await self._push_event_to_agent(label, desc, person_name, event_images)
            print(
                f"\033[91m[SYSTEM_EVENT] [{label}] {desc} (confirmed)\033[0m",
                flush=True,
            )
            self._mark_langfuse_event_pushed(state, chunk_index, label, desc, person_name)
        except Exception as exc:
            LOGGER.warning("[%s] confirm task failed: %s", label, exc)
        finally:
            state._confirm_pending.discard(label)

    def _confirm_frame_refs(self, state: SessionState, max_frames: int = 3) -> list[str]:
        """确认复核使用的最近几帧：取当前 chunk 已累积帧的尾部，不足时回落最新帧。"""
        frames = [str(ip) for ip in state.current_chunk.get("image_paths", []) if ip]
        if not frames and state.latest_image:
            frames = [state.latest_image]
        return frames[-max_frames:]

    def _disable_thinking_in_kwargs(self, kwargs: dict[str, Any]) -> None:
        """Force-disable thinking for the confirm call regardless of config."""
        extra_body = kwargs.get("extra_body")
        if not isinstance(extra_body, dict):
            extra_body = {}
            kwargs["extra_body"] = extra_body
        if "dashscope" in (self.config.main_api_base or "").lower():
            extra_body["enable_thinking"] = False
        else:
            extra_body.setdefault("chat_template_kwargs", {}).update(
                {"enable_thinking": False, "enable_thinking_assert": False}
            )

    async def _confirm_event(self, label: str, image_refs: Optional[list[str]]) -> bool:
        """Multi-frame recheck: do the freshest frames still show the event?

        Uses a fresh standalone chat (system + these frames only), with no
        x-streaming-session header, so it never pollutes the main detection
        session's context. Thinking is force-disabled.
        """
        if not image_refs:
            return False
        prompt = EVENT_CONFIRM_PROMPTS.get(label)
        if not prompt:
            return False
        content: list[dict[str, Any]] = []
        for image_ref in image_refs:
            ...
            if os.path.isfile(image_ref):
                image_ref = _file_to_data_url(image_ref)
            if image_ref:
                content.append({"type": "image_url", "image_url": {"url": image_ref}})
        if not content:
            return False
        content.append({"type": "text", "text": "请输出确认结果 JSON。"})
        messages: list[dict[str, Any]] = [
            {"role": "system", "content": prompt},
            {"role": "user", "content": content},
        ]
        kwargs = self._main_generation_kwargs({})
        self._disable_thinking_in_kwargs(kwargs)
        # 服务端可能忽略 thinking 关闭、把推理内联进 content；给足 token 让模型
        # 能推理完并输出 JSON，再取最后一次出现的 confirm 值。
        ...
        try:
            ...
            LOGGER.info("[%s] confirm response: %r", label, text[:60])
            confirmed = _extract_confirm_bool(text)
            if confirmed is not None:
                return confirmed
            lowered = text.lower()
            return lowered.startswith("yes") or "是" in text[:4]
        except Exception as exc:
            LOGGER.warning("[%s] confirm inference failed: %s", label, exc)
            return False

    def _langfuse_record_available(self) -> bool:
        if not self.config.langfuse_chunk_record_enabled:
            return False
        try:
            client = get_client()
    ...
    def _mark_langfuse_event_pushed(
        self,
        state: SessionState,
        ...
        """Register a successfully pushed event for the chunk it was detected in.

        Called after an event is actually pushed (direct push or confirm passed).
        The chunk snapshot is recorded at chunk-flush time (or immediately if the
        chunk already flushed when the async confirm completes).
        """
        if not self._langfuse_record_available():
            return
        if chunk_index is None:
            chunk_index = state.chunk_index
        state.langfuse_pushed_events.setdefault(chunk_index, []).append(
            {
                "label": label,
                "desc": desc,
                "name": person_name,
        ...
        if chunk_index < state.chunk_index:
            # Chunk already flushed while the confirm was pending; record now.
            asyncio.ensure_future(
                asyncio.to_thread(self._record_chunk_to_langfuse, state, chunk_index)
            )

    async def _record_flushed_chunk_if_event(self, state: SessionState) -> None:
        """Record the just-flushed chunk to Langfuse if it had pushed events."""
        if not self._langfuse_record_available():
            return
        chunk_index = state.chunk_index
        if state.langfuse_pushed_events.get(chunk_index):
            await asyncio.to_thread(self._record_chunk_to_langfuse, state, chunk_index)

    def _record_chunk_to_langfuse(self, state: SessionState, chunk_index: int) -> None:
        events = state.langfuse_pushed_events.pop(chunk_index, None)
        if not events:
            return
        try:
            self._write_event_chunk_trace(state, chunk_index, events)
        except Exception as exc:
            LOGGER.warning(
                "[%s] langfuse chunk record failed (chunk=%s): %s",
                state.session_id,
                chunk_index,
                exc,
            )

    def _write_event_chunk_trace(
        self,
        state: SessionState,
        chunk_index: int,
        events: list[dict[str, Any]],
    ) -> None:
        """Write one Langfuse trace per event-triggered chunk.

        Trace input carries the chunk context (system prompt, chunk meta, events);
        each turn of the chunk becomes a generation with its user message (base64
        images attached) as input and the model output as output.
        """
        chunk_predictions = [
            p
            for p in state.predictions
            if p.get("input", {}).get("chunk_index") == chunk_index
        ]
        if not chunk_predictions:
            LOGGER.info(
                "[%s] no predictions for chunk=%s; skip langfuse record",
                state.session_id,
                chunk_index,
            )
            return
        ...
        for p in chunk_predictions:
            tr = p.get("input", {}).get("frame_time_ranges")
            if tr:
                frame_time_ranges.extend(tr)
        frame_range_text = (
            f"{frame_time_ranges[0]} ~ {frame_time_ranges[-1]}"
            if frame_time_ranges
            else ""
        )

        # Ordered, labeled frame list (turn 1..N) so the trace root span shows
        # every frame in one place. Keep inline base64: media upload is disabled
        # because the server's presigned URL points at an unreachable
        # Docker-internal MinIO.
        pushed_labels = {e["label"] for e in events}
        frames: list[dict[str, Any]] = []
        for turn_no, pred in enumerate(chunk_predictions, start=1):
            turn_input = pred.get("input", {})
            user_message = turn_input.get("vllm_message") or {}
            content_items = self._langfuse_message_content(
            ...
            ev = extract_event(pred.get("prediction", ""))
            if ev and ev[0] in pushed_labels:
                triggered_event = {
                    "label": ev[0],
                    "desc": ev[1],
                }
            frames.append(
                {
                    "turn": pred.get("turn", turn_no),
                    "time_range": pred.get("time_range", ""),
                    "captured_at": (turn_input.get("frame_captured_at") or [""])[0],
                    "image": image_url,
        ...
        trace_input = {
            ...
            "frame_range": frame_range_text,
            "frame_count": len(frame_time_ranges),
            "turn_count": len(chunk_predictions),
            "frames": frames,
            "events": [
                {
                    "label": e["label"],
                    "desc": e["desc"],
                    "name": e["name"],
                    "pushed_at": _iso_time(e.get("pushed_at")),
                }
                for e in events
            ],
        ...
        with client.start_as_current_observation(
            ...
            name="event_chunk",
            input=trace_input,
            metadata={"chunk_index": chunk_index, "session_id": state.session_id},
        ) as root_span:
                ...
                for turn_no, pred in enumerate(chunk_predictions, start=1):
                    ...
                    with root_span.start_as_current_observation(
                        ...
                        metadata={
                            ...
                            "turn": pred.get("turn"),
                            "time_range": pred.get("time_range", ""),
                            "frame_time_ranges": turn_input.get("frame_time_ranges", []),
                        },
                    ):
                        pass
        client.flush()

    def _langfuse_message_content(
        self,
        content: Any,
    ) -> list[dict[str, Any]]:
        """Convert an internal vllm_message content list into Langfuse-friendly
        items with images attached inline (media upload is disabled)."""
        ...
        for item in content:
            ...
            if item_type == "image":
                ...
                elif isinstance(image_ref, str) and os.path.isfile(image_ref):
                    # 内联 base64：随 trace 上传，避免依赖外部媒体服务直传
                    items.append(
                        {
                            "type": "image",
                            "image": _file_to_data_url(image_ref),
                        }
                    )
                else:
                    items.append({"type": "text", "text": f"[image:{image_ref}]"})
            elif item_type == "text":
    ...
    async def _push_system_event(
        ...
        url = self.config.system_event_url.strip()
        if not url:
            return
        ...
        if images:
            ...
            if img_bytes:
                self._save_event_image(event_label, img_bytes)
        payload: dict[str, Any] = {
            "event": event_label,
            "description": description,
            "level": _EVENT_LEVELS.get(event_label, "alert"),
            "name": person_name,
            "images": image_b64_list,
            "videos": [],
        }
        status = await _post_event_json(url, payload)
        if status is not None and status < 400:
            LOGGER.info("system event pushed: [%s] (status=%s)", event_label, status)
        elif status is not None:
            LOGGER.warning("system event endpoint returned %s: [%s]", status, event_label)

    async def _push_event_to_agent(
        self,
        event_label: str,
        description: str,
        person_name: str = "",
        ...
        """POST a detection event to the omni-agent event server (POST /event).

        Routing matches omin_agent events/server.py:
        - 普通手势（ACTION_EVENT_NAMES）→ event_type="action"，走 agent 动作管道；
        - 其它事件（身体不适/环境/跌倒等）→ event_type=英文标签，走通用系统事件管道。
        """
        url = self.config.agent_webhook_url.strip()
        if not url:
            return
        # agent 端只读取 description/event_name，不认 name 字段；
        # 因此把人员姓名拼进 description（保持原描述不变，仅追加“（人员：xxx）”）。
        agent_description = description
        if person_name:
            agent_description = f"{description}（人员：{person_name}）"
        image_b64 = ""
        ...
        payload: dict[str, Any]
        action_name = ACTION_EVENT_NAMES.get(event_label)
        if action_name:
            # 普通手势 → 动作管道
            payload = {
                "event_type": "action",
            ...
            log_label = f"action:{action_name}"
        else:
            # 其它事件 → 通用系统事件（event_type 用英文标签）
            payload = {
            ...
            log_label = event_label

        status = await _post_event_json(url, payload)
        if status is not None and status < 400:
            LOGGER.info("event pushed to agent: [%s] (status=%s)", log_label, status)
        elif status is not None:
            LOGGER.warning("agent webhook returned %s: [%s]", status, log_label)

    def _save_event_image(self, event_label: str, image_bytes: bytes) -> None:
        try:
            save_dir = Path(self.config.frame_save_dir) / "events"
            save_dir.mkdir(parents=True, exist_ok=True)
            ts = datetime.now().strftime("%Y%m%d_%H%M%S")
            # Truncate label to avoid filename-too-long errors (ext4 limit: 255 bytes)
            safe_label = event_label[:40].replace("/", "_").replace("\x00", "")
            filename = f"event_{safe_label}_{ts}.jpg"
            path = save_dir / filename
            with open(path, "wb") as f:
                f.write(image_bytes)
            LOGGER.info("event image saved: %s", path)
        except Exception as exc:
            LOGGER.warning("failed to save event image: %s", exc)
    ...
    async def _call_main_model(
        ...
        *,
        ...
        if self.config.debug_print_vlm_messages:
            _print_vlm_messages(api_messages, model_name, generation_kwargs)
        response = await client.chat.completions.create(
            model=model_name,
    ...
    def _main_generation_kwargs(self, inbound_payload: dict[str, Any]) -> dict[str, Any]:
        ...
        if self.config.main_disable_thinking:
            if "dashscope" in (self.config.main_api_base or "").lower():
                # DashScope: enable_thinking 是 extra_body 顶层字段
                extra_body["enable_thinking"] = False
            else:
                # vLLM/SGLang: 放在 chat_template_kwargs 里
                extra_body.setdefault("chat_template_kwargs", {}).update(
                    {"enable_thinking": False, "enable_thinking_assert": False}
                )
    ...
    async def _flush_chunk(self, state: SessionState, use_async_summary: bool = False) -> None:
        ...
        if self.summarizer is not None and use_async_summary:
            ...
            try:
                ...
                LOGGER.warning(
                    "[%s] chunk=%s mid-summary failed: %r; skipping",
                    state.session_id,
                    state.chunk_index,
                    exc,
                )
            ...
            self._append_summary_log(state, "mid_term", mid_term_entry)
            LOGGER.info(
            ...
            if len(state.mid_term_summaries) >= self.config.compress_every_n_chunks:
                await self._safe_compress_mid_terms(state)
    ...
    def _build_mid_term_summary_entry(
        ...
        start = time.time()
        summary, mid_usage, mid_term_debug_input = self.summarizer.generate_detailed_summary(
        ...
        entry = {
            ...
            "captured_at_start": (
                chunk_snapshot.get("frame_captured_at") or [None]
            )[0],
            "captured_at_end": (
                chunk_snapshot.get("frame_captured_at") or [None]
            )[-1],
    ...
    def _compress_mid_terms(self, state: SessionState) -> None:
        ...
        start = time.time()
        merged, lt_usage, compressed_text, long_term_debug_input = self.summarizer.batch_compress_to_longterm(
        ...
        self._append_summary_log(state, "long_term", long_term_entry)
        ...
        LOGGER.info(
            "[%s] long-term compression batch=%d %.3fs tokens=%s",
            state.session_id,
            batch_index,
            elapsed,
            lt_usage,
    ...
    async def _safe_compress_mid_terms(self, state: SessionState) -> None:
        """Submit long-term compression without waiting in the detection turn."""
        if state.long_term_compression_task is not None:
            return
        if len(state.mid_term_summaries) < self.config.compress_every_n_chunks:
            return

        batch_entries = list(state.mid_term_summaries)
        state.mid_term_summaries.clear()
        batch_index = state.long_term_compression_next_index
        state.long_term_compression_next_index += 1
        existing_longterm = state.memory_state["long_term_memory"]

        async def run_compression() -> None:
            try:
                start = time.time()
                merged, usage, compressed_text, debug_input = await asyncio.to_thread(
                    self.summarizer.batch_compress_to_longterm,
                    existing_longterm,
                    batch_entries,
                )
                elapsed = time.time() - start
                source_frame_ranges = [e["frame_range"] for e in batch_entries]
                state.memory_state["long_term_memory"] = merged
                ...
                long_term_entry = {
                    ...
                    "query": state.current_query_text,
                    "query_start_time": state.query_start_time,
                    "source_chunk_indices": [e["chunk_index"] for e in batch_entries],
                    "source_frame_ranges": source_frame_ranges,
                    "source_summary_count": len(source_frame_ranges),
                    "compressed_text": compressed_text,
                    "inference_time": round(elapsed, 3),
                ...
                debug_input_path = self._save_summarizer_debug_input(
                    state, "long_term", batch_index, copy.deepcopy(debug_input),
                )
                if debug_input_path:
                    long_term_entry["debug_input_path"] = debug_input_path
                state.long_term_history.append(long_term_entry)
                ...
                window = int(self.config.long_term_memory_window or 0)
                if window > 0 and len(state.long_term_history) > window:
                    del state.long_term_history[:-window]
                    state.memory_state["long_term_memory"] = "\n\n".join(
                        e["compressed_text"].rstrip()
                        for e in state.long_term_history
                        if e.get("compressed_text")
                    )
                LOGGER.info(
                    "[%s] background long-term compression batch=%d %.3fs tokens=%s",
                    state.session_id, batch_index, elapsed, usage,
                )
            except asyncio.CancelledError:
                raise
            except Exception as exc:
                state.mid_term_summaries[0:0] = batch_entries
                LOGGER.warning(
                    "[%s] background long-term compression failed: %r; batch restored",
                    state.session_id, exc,
                )
            finally:
                state.long_term_compression_task = None
                if len(state.mid_term_summaries) >= self.config.compress_every_n_chunks:
                    await self._safe_compress_mid_terms(state)

        state.long_term_compression_task = asyncio.create_task(run_compression())
        LOGGER.info(
            "[%s] submitted background long-term compression batch=%d summaries=%d",
            state.session_id, batch_index, len(batch_entries),
        )
    ...
    def _append_summary_log(
        ...
        """Append one completed summary to a dedicated JSONL log."""
        configured_path = self.config.summary_log_path.strip()
        ...
        summary_text = (
            ...
            if stage == "mid_term"
            else entry.get("compressed_text", "")
        )
        record = {
            ...
            "source_frame_ranges": entry.get("source_frame_ranges"),
            "frame_count": entry.get("frame_count"),
            "key_frame_count": entry.get("key_frame_count"),
            "captured_at_start": entry.get("captured_at_start"),
            "captured_at_end": entry.get("captured_at_end"),
            "inference_time": entry.get("inference_time"),
            "summary": summary_text,
            "token_usage": entry.get("token_usage"),
        }
        try:
            with self._summary_log_lock:
                path.parent.mkdir(parents=True, exist_ok=True)
                with path.open("a", encoding="utf-8") as log_file:
                    log_file.write(json.dumps(record, ensure_ascii=False) + "\n")
        except OSError as exc:
            LOGGER.warning("failed to append summary log %s: %s", path, exc)
    ...
    def _append_async_summary_user_message(
        ...
        *,
        ...
        for tr, ip, captured_at in zip(time_ranges, image_paths, captured_at_values):
            ...
            segment["frame_captured_at"].append(captured_at)
            segment["summarizer_frame_cache"].append(
                {"path": ip, "captured_at": captured_at}
            )
    ...
    async def _commit_required_async_summaries(
        ...
        while state.async_pending_summary_jobs:
            ...
            try:
                ...
                LOGGER.warning(
                    "[%s] async mid-summary %s failed: %r; skipping",
                    state.session_id,
                    job.get("summary_index"),
                    exc,
                )
...
# 事件推送超时：确保推送失败/超时绝不会阻塞主程序事件循环
_EVENT_PUSH_TOTAL_TIMEOUT_SECONDS = 5.0
_EVENT_PUSH_CONNECT_TIMEOUT_SECONDS = 3.0


async def _post_event_json(url: str, payload: dict[str, Any]) -> Optional[int]:
    """POST JSON to an event endpoint with a hard wall-clock bound.

    Never raises. Returns the HTTP status code on success, or None on any
    timeout/failure (already logged).  The hard ``asyncio.wait_for`` bound
    guarantees a stuck peer can never stall the caller, and the inner
    ClientTimeout cancels a dead connect after ``_EVENT_PUSH_CONNECT_TIMEOUT``.
    """
    import aiohttp

    async def _do_post() -> int:
        timeout = aiohttp.ClientTimeout(
            total=_EVENT_PUSH_TOTAL_TIMEOUT_SECONDS,
            connect=_EVENT_PUSH_CONNECT_TIMEOUT_SECONDS,
            sock_connect=_EVENT_PUSH_CONNECT_TIMEOUT_SECONDS,
        )
        async with aiohttp.ClientSession() as session:
    ...
    try:
        return await asyncio.wait_for(
            _do_post(), timeout=_EVENT_PUSH_TOTAL_TIMEOUT_SECONDS
        )
    except asyncio.TimeoutError:
        LOGGER.warning(
            "event push to %s timed out after %.0fs",
            url,
            _EVENT_PUSH_TOTAL_TIMEOUT_SECONDS,
        )
    except Exception as exc:
        LOGGER.warning("event push to %s failed: %r", url, exc)
    return None
...
def _extract_frame_captured_at_values(
    request: web.Request,
    payload: dict[str, Any],
    frame_count: int,
) -> list[str]:
    """Return one wall-clock timestamp per frame; relative time remains authoritative."""
    raw_values = payload.get("frame_captured_at")
    values = [str(v).strip() for v in raw_values] if isinstance(raw_values, list) else []
    if not values:
        single = (
            request.headers.get("x-frame-captured-at")
            or str(payload.get("x_frame_captured_at") or "")
        ).strip()
        if single:
            values = [single]
    fallback = datetime.now().astimezone().isoformat(timespec="milliseconds")
    return [values[i] if i < len(values) and values[i] else fallback for i in range(frame_count)]
...
def parse_args() -> AdapterConfig:
    ...
    parser.add_argument(
        "--max-qa-entries",
        type=int,
        default=_env_int("MAX_QA_ENTRIES", 20),
        help="Maximum number of QA history entries to include in the prompt.",
    )
    parser.add_argument(
        "--max-prefix-chars",
        type=int,
        default=_env_int("MAX_PREFIX_CHARS", 8000),
        help="Maximum number of characters for the prefix content injected into the first user message.",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--summary-log-path",
        default=os.environ.get("SUMMARY_LOG_PATH", ""),
        help="Append completed mid/long-term summaries as JSONL to this file.",
    )
    parser.add_argument(
        "--save-root",
        default=os.environ.get(
    ...
    parser.add_argument(
        "--debug-print-vlm-messages",
        action="store_true",
        default=_env_bool("DEBUG_PRINT_VLM_MESSAGES", False),
        help="Print the full messages sent to the main VLM on every inference call.",
    )
    parser.add_argument(
        "--system-prompt",
        default=os.environ.get("SYSTEM_PROMPT", DEFAULT_SYSTEM_PROMPT_EN),
        help="System prompt prepended to every 8B main-model request.",
    )
    parser.add_argument(
        "--language",
    ...
    parser.add_argument(
        "--system-event-url",
        default=os.environ.get("SYSTEM_EVENT_URL", "http://127.0.0.1:8770/system_event"),
        help="When VLM detects an event (</response>), POST the event to this system event URL.",
    )
    parser.add_argument(
        "--agent-webhook-url",
        default=os.environ.get("AGENT_WEBHOOK_URL", "http://192.168.217.100:8769/event"),
        help="Also POST detected events to this Agent webhook URL.",
    )
    parser.add_argument(
        "--system-event-labels",
        default=os.environ.get("SYSTEM_EVENT_LABELS", ""),
        help="Comma-separated event push allowlist; empty allows every known event.",
    )
    parser.add_argument(
        "--time-api-url",
        default=os.environ.get("TIME_API_URL", ""),
        help="Remote time API URL for clock correction. JSON with 'datetime' field. Empty = use local time.",
    )
    parser.add_argument(
        "--night-time-start",
        type=int,
        default=int(os.environ.get("NIGHT_TIME_START", "22")),
        help="Start hour of night period (default: 22, i.e. 10pm).",
    )
    parser.add_argument(
        "--night-time-end",
        type=int,
        default=int(os.environ.get("NIGHT_TIME_END", "6")),
        help="End hour of night period (default: 6, i.e. 6am).",
    )
    ...
    return AdapterConfig(
        ...
        debug_print_vlm_messages=args.debug_print_vlm_messages,
        frame_save_dir=args.frame_save_dir,
        language=args.language,
        system_prompt=args.system_prompt,
        system_event_url=args.system_event_url,
        agent_webhook_url=args.agent_webhook_url,
        system_event_labels=args.system_event_labels,
        time_api_url=args.time_api_url,
        night_time_start=args.night_time_start,
        night_time_end=args.night_time_end,
    )
