...
ROSBRIDGE_URL = os.getenv("ROSBRIDGE_URL", "ws://192.168.217.100:9090")
...
class LocalCameraCapture:
    """本地摄像头单帧捕获，接口与 FoxgloveImageCapture 一致。"""
    ...
    def read(self, timeout: float = 1.0) -> Tuple[bool, Optional[np.ndarray]]:
        ...
        if not self._cap.isOpened():
            logger.error(f"[local_camera] 无法打开摄像头 /dev/video{self._camera_index}")
            return False, None
...
class RosbridgeImageCapture:
    ...
    接口与 FoxgloveImageCapture 完全一致:
        cap = RosbridgeImageCapture(topic)
        success, img = cap.read()   # img 是 RGB numpy 数组
        cap.release()
    """
