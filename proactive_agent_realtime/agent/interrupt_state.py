...
_tool_audio_active = threading.Event()  # 本地 CLI 命令执行期间置真（如唱歌），复用「AI 讲话」的打断语义
...
def set_tool_audio_active(v: bool) -> None:
    """本地 CLI 命令（如唱歌）执行期间置真，借用「AI 讲话」的打断语义：关流 + KWS 独占队列听打断词。"""
    if v:
        _tool_audio_active.set()
    else:
        _tool_audio_active.clear()


def is_ai_speaking() -> bool:
    """KWS 线程 / feeder 门控读取：AI 是否正在讲话（含本地 CLI 命令执行）。"""
    return _ai_speaking.is_set() or _tool_audio_active.is_set()


def is_tool_audio_active() -> bool:
    """本地 CLI 命令（如唱歌）是否正在执行 —— 任何模式下都需 KWS 独占队列听打断词。"""
    return _tool_audio_active.is_set()
