...
from agent.interrupt_state import is_tool_audio_active
...
class WakeupDetector:
    ...
    def _generate_keywords_file(self):
        ...
        # 唤醒词与打断词可能相同（dict.fromkeys 去重，避免 keywords 文件出现重复行）
    ...
    def _should_yield(self, asr_active: bool, speaking: bool) -> bool:
        ...
        - 本地 CLI 命令执行中（唱歌等）：任何模式都不让出，KWS 独占队列听打断词。
        - keyword 模式：AI 讲话时 KWS 需继续消费听打断词，只在「ASR 激活且未讲话」时让出。
        - vad 模式：唤醒后（ASR 激活）或 AI 讲话时都让出，打断靠服务端 VAD。
        """
        if is_tool_audio_active():
            return False
    ...
    def _worker_loop(self, audio_queue):
        ...
        while self.is_running:
            try:
                ...
                # 检测到关键词的瞬间重新读说话状态
                speaking = self._is_speaking()
                is_interrupt = result == self._interrupt_keyword
                is_wakeup = result == WAKEUP_KEYWORD

                # 命中打断词 → 触发打断：
                #   · 本地命令执行中（唱歌等）：任何模式都打断
                #   · keyword 模式：AI 讲话中命中打断词（打断词==唤醒词时同理）
                if is_interrupt and (is_tool_audio_active() or (speaking and INTERRUPT_MODE == "keyword")):
                    if self._interrupted_event is not None:
                        logger.info(f"[唤醒检测] 检测到打断词 '{result}'，触发打断")
                ...
                # 未讲话命中打断词（且打断词 ≠ 唤醒词）→ 忽略
                if is_interrupt and not is_wakeup:
