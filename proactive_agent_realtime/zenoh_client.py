"""以 FoxgloveImageCapture 风格读取 Zenoh 标注图像。

本文件可单独交付；客户侧只需安装 ``eclipse-zenoh``、``opencv-python`` 和
``numpy``，不依赖 ZenohRelay 的其他源码。
"""
...
DEFAULT_URL = "tcp/127.0.0.1:7447"
_META_MAGIC = b"ZRM1"
...
class ZenohImageCapture:
    ...
    客户端不选择本地摄像头或 ROS/Foxglove 话题；这些输入源只在服务端配置。
    ``topic`` 是服务端输出的 Zenoh key，通常保持默认值即可；``url`` 是服务端
    固定端口 endpoint（如 ``tcp/192.168.1.10:7447``），默认连接本机固定端口。
    ``fps`` 是客户侧最大取用频率，0 表示不限；``read`` 返回 RGB numpy 图像。
    """
    ...
    def __init__(
        ...
        self._topic = topic
        self._url = url if url is not None else (os.getenv("ZENOH_URL") or DEFAULT_URL)
        self._fps = fps
