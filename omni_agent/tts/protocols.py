...
class Message:
    ...
    def _get_readers(self) -> List[Callable[[io.BytesIO], None]]:
        """Get list of reader functions (order must match _get_writers)."""
        readers = []
...
async def task_request(
    ...
    msg.session_id = session_id
    msg.payload = payload
    logger.debug(f"Sending task_request: {msg}")
    await websocket.send(msg.marshal())
...
async def chat_tts_text(
    ...
    """Send streaming TTS text (ChatTTSText event, appends to current synthesis)."""
    msg = Message(type=MsgType.FullClientRequest, flag=MsgTypeFlagBits.WithEvent)
    msg.event = EventType.ChatTTSText
    msg.session_id = session_id
    msg.payload = payload
    logger.debug(f"Sending chat_tts_text: {msg}")
    await websocket.send(msg.marshal())
