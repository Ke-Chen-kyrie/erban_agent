...
class AdapterConfig:
    ...
    system_event_url: str = os.environ.get("SYSTEM_EVENT_URL", "http://127.0.0.1:8769/system_event")
...
class StreamingInferAdapter:
    ...
    async def _handle_chat_payload(
        ...
        *,
        ...
        if response_payload and self.config.system_event_url:
            parts = response_payload.split(None, 1)
            label = parts[0].strip("[]") if parts else response_payload
            desc = parts[1] if len(parts) > 1 else ""
            asyncio.ensure_future(self._push_system_event(label, desc, [str(ip) for ip in image_paths]))
    ...
    async def _push_system_event(self, event_label: str, description: str, images: Optional[list[str]] = None) -> None:
        ...
        try:
            ...
            async with aiohttp.ClientSession() as session:
                async with session.post(url, json=payload, timeout=aiohttp.ClientTimeout(total=5)) as resp:
                    if resp.status < 400:
                        LOGGER.info("system event pushed: [%s] (status=%s)", event_label, resp.status)
                    else:
                        LOGGER.warning("system event endpoint returned %s: [%s]", resp.status, event_label)
        except Exception as exc:
            LOGGER.warning("failed to push system event to %s: %s", url, exc)
    ...
    def _save_event_image(self, event_label: str, image_bytes: bytes) -> None:
        try:
            ...
            filename = f"event_{event_label}_{ts}.jpg"
...
def parse_args() -> AdapterConfig:
    ...
    parser.add_argument(
        "--system-event-url",
        default=os.environ.get("SYSTEM_EVENT_URL", "http://127.0.0.1:8769/system_event"),
