...
from agent.tools import build_tools_for_realtime, execute_shell, execute_tool
from agent.voice_state import get_current_voice
from agent.interrupt_state import consume_interrupt, is_ai_speaking, set_ai_speaking, reset as reset_interrupt_state
...
class RealtimeSession:
    ...
    async def _feed_audio_and_video(self, audio_queue: queue.Queue):
        ...
        while not self._stop_event.is_set():
            # keyword 模式：AI 讲话时关流，让 KWS 独占队列听打断词
            if INTERRUPT_MODE == "keyword" and is_ai_speaking():
